...
void HDMITX_InitTxDev(_CODE HDMITXDEV *pInstance)
{
	...
	{
		hdmiTxDev[gHDMI_TxIndex] = *pInstance ;
	}
}
...
void InitHDMITX()
{
    ...
    //HDMITX_WriteI2C_Byte(REG_TX_INT_CTRL,hdmiTxDev[gHDMI_TxIndex].bIntType);
    hdmiTxDev[gHDMI_TxIndex].bIntPOL = (hdmiTxDev[gHDMI_TxIndex].bIntType&B_TX_INTPOL_ACTH)?TRUE:FALSE ;
    ...
    hdmiTxDev[gHDMI_TxIndex].RCLK = CalcRCLK();
#endif
...
{
    BYTE intdata1,intdata3,sysstat; 
#ifdef SUPPORT_HDCP
    BYTE  intdata2 = 0 ; 
#endif     
    
    BYTE  intclr3 = 0 ;
    BYTE PrevHPD = hdmiTxDev[gHDMI_TxIndex].bHPD ;
    BYTE HPD ;
    
    sysstat = HDMITX_ReadI2C_Byte(REG_TX_SYS_STATUS);
//	HDMITX_DEBUG_PRINTF(("#441 CheckHDMITX()  REG_TX_SYS_STATUS = 0x%02x \n",(int)sysstat));
    ...
    if(HPD==FALSE)
    {
        hdmiTxDev[gHDMI_TxIndex].bAuthenticated = FALSE ;
    }
    ...
    if(sysstat & B_TX_INT_ACTIVE)
    {
		...
		if(intdata1 & B_TX_INT_DDC_BUS_HANG)
		{
            ...
            if(hdmiTxDev[gHDMI_TxIndex].bAuthenticated)
            {
        ...
        //HDMITX_DEBUG_PRINTF(("INT_Handler: reg%X = %X\n",(int)REG_TX_INT_STAT2,(int)HDMITX_ReadI2C_Byte(REG_TX_INT_STAT2); ));

		#ifdef SUPPORT_HDCP
		if(intdata2 & B_TX_INT_AUTH_DONE)
		{
            //HDMITX_DEBUG_PRINTF(("interrupt Authenticate Done. bAuthenticated = TRUE \n"));
            HDMITX_OrReg_Byte(REG_TX_INT_MASK2,(BYTE)B_TX_AUTH_DONE_MASK);
            hdmiTxDev[gHDMI_TxIndex].bAuthenticated = TRUE ;  
		...
		if(intdata2 & B_TX_INT_AUTH_FAIL)
		{
            ...
            //hdmitx_hdcp_ResumeAuthentication(); //chunyu
        }
        #endif // SUPPORT_HDCP
    ...
    hdmiTxDev[gHDMI_TxIndex].bHPD = HPD ;
    return HPD ;
}
...
//chunyu@start
	if (bEnable == 1  )
	{		
		HDMITX_WriteI2C_Byte(REG_TX_AV_MUTE , 0x03 );
        //HDMITX_DEBUG_PRINTF(("setHDMITX_AVMute  TRUE.\n"));
	}
	else 
	{
		HDMITX_WriteI2C_Byte(REG_TX_AV_MUTE , 0x02 );		
        //HDMITX_DEBUG_PRINTF(("setHDMITX_AVMute  FALSE.\n"));
	}
//chunyu@end
...
//        HDMITX_DEBUG_PRINTF(("getHDMITX_EDIDBytes(): Invallid pData pointer %08lX\n",(ULONG)pData));
    ...
    while(RemainedCount > 0)
    {
        ...
        EDID_DEBUG_PRINTF(("getHDMITX_EDIDBytes(): ReqCount = %d,bCurrOffset = %d\r\n",(int)ReqCount,(int)bCurrOffset));
        ...
        for(TimeOut = 0 ; TimeOut < 200 ; TimeOut++)
        {
            ...
            ucdata1 = HDMITX_ReadI2C_Byte(REG_TX_INT_STAT1) & B_TX_INT_DDC_BUS_HANG ;
            if((ucdata & B_TX_DDC_ERROR) || ucdata1)
            {
                HDMITX_DEBUG_PRINTF(("rREG_TX_DDC_STATUS=0x%02x ,ucdata1=0x%02x  hdmitx_AbortDDC\r\n",ucdata,ucdata1)) ;
                //HDMITX_DEBUG_PRINTF(("Called hdmitx_AboutDDC()\n"));
                hdmitx_AbortDDC();
                return ER_FAIL ;
            }
        ...
        if(TimeOut == 0)
        {
            ...
            // HDMITX_AndReg_Byte(REG_TX_INT_CTRL,~(1<<1));  //chunyu
            return ER_FAIL ;
        }
        do
        {
...
{
    BYTE SWReset ; // CPDesire,DDCMaster ;
    BYTE uc, timeout, i ;
...
//                HDMITX_DEBUG_PRINTF(("hdmitx_AbortDDC Fail by reg16=%02X\n",(int)uc));
            ...
            delayTX1ms(1); // delay 1 ms to stable.
        }
    }
...
{
    // bInputVideoMode,bOutputVideoMode,hdmiTxDev[gHDMI_TxIndex].bInputVideoSignalType,bAudioInputType,should be configured by upper F/W or loaded from EEPROM.
    ...
    hdmiTxDev[gHDMI_TxIndex].bHDMIMode = (BYTE)bHDMI ;
...
/*
    if(hdmiTxDev[gHDMI_TxIndex].bHDMIMode)
    {
        setHDMITX_AVMute(TRUE);	   //chunyu
		//printf("Test HDMITX_EnableVideoOutput HDMIMode \n ") ; 
    }
*/

    hdmitx_SetInputMode(inputColorMode,hdmiTxDev[gHDMI_TxIndex].bInputVideoSignalType);
...
//		bAudioEnable = TRUE ;  //chunyu
		setHDMITX_AudioChannelEnable(TRUE); //chunyu		
		//HDMITX_DEBUG_PRINTF(("HDMITX_EnableVideoOutput ,HDMI Mode  \n"));
        HDMITX_VXIS_DEBUG_PRINTF(("HDMITX_EnableVideoOutput(gHDMI_TxIndex=%d),HDMI Mode\r\n",(int)gHDMI_TxIndex ));
    }
    else
    {
        HDMITX_WriteI2C_Byte(REG_TX_HDMI_MODE,B_TX_DVI_MODE);		
		//HDMITX_DEBUG_PRINTF(("HDMITX_EnableVideoOutput ,DVI Mode  \n"));
        HDMITX_VXIS_DEBUG_PRINTF(("HDMITX_EnableVideoOutput(gHDMI_TxIndex=%d),DVI Mode\r\n",(int)gHDMI_TxIndex ));
    }
...
//    HDMITX_WriteI2C_Byte(REG_TX_SW_RST,          B_HDMITX_AUD_RST|B_TX_AREF_RST|B_TX_HDCP_RST_HDMITX);  //chunyu
...
{
	hdmiTxDev[gHDMI_TxIndex].bInputVideoSignalType = inputSignalType ;
  // hdmitx_SetInputMode(inputColorMode,hdmiTxDev[gHDMI_TxIndex].bInputVideoSignalType);
    return TRUE ;
}
...
#ifdef SUPPORT_OUTPUTYUV  
    BYTE i ;
#endif 

    csc = B_HDMITX_CSC_BYPASS ;  //init    
...
void hdmitx_SetupAFE(VIDEOPCLKLEVEL level)
{
    ...
    HDMITX_SetI2C_Byte(REG_TX_SW_RST,B_TX_REF_RST_HDMITX|B_HDMITX_VID_RST,0);    //chunyu
...
void setHDMITX_LPCMAudio(BYTE AudioSrcNum, BYTE AudSWL, BOOL bSPDIF)
{
    ...
    AudioEnable = 0 ;
    AudioFormat = hdmiTxDev[gHDMI_TxIndex].bOutputAudioMode ;
    ...
    hdmiTxDev[gHDMI_TxIndex].bAudioChannelEnable=AudioEnable;
...
void setHDMITX_HBRAudio(BOOL bSPDIF)
{
    ...
    hdmiTxDev[gHDMI_TxIndex].bAudioChannelEnable=HDMITX_ReadI2C_Byte(REG_TX_AUDIO_CTRL0);
...
void HDMITX_EnableAudioOutput(BYTE AudioType, BOOL bSPDIF,  ULONG SampleFreq,  BYTE ChNum, BYTE *pIEC60958ChStat, ULONG TMDSClock)
{
    ...
    AudioDelayCnt=36;
    LastRefaudfreqnum=0;
    hdmiTxDev[gHDMI_TxIndex].TMDSClock=TMDSClock;
    hdmiTxDev[gHDMI_TxIndex].bAudioChannelEnable=0;
    hdmiTxDev[gHDMI_TxIndex].bSPDIF_OUT=bSPDIF;
    ...
    if( AudioType != T_AUDIO_DSD)
    {
        ...
        hdmiTxDev[gHDMI_TxIndex].bAudFs=AUDFS_OTHER;
    #else
        hdmiTxDev[gHDMI_TxIndex].bAudFs=Fs;
    #endif
        setHDMITX_NCTS(hdmiTxDev[gHDMI_TxIndex].bAudFs);
    ...
    HDMITX_WriteI2C_Byte(REG_TX_AUDIO_CTRL0, hdmiTxDev[gHDMI_TxIndex].bAudioChannelEnable);
...
void hdmitx_AutoAdjustAudio()
{
    ...
    cTMDSClock = hdmiTxDev[gHDMI_TxIndex].TMDSClock ;
    ...
    if(hdmiTxDev[gHDMI_TxIndex].bAudFs != fs)
    {
        hdmiTxDev[gHDMI_TxIndex].bAudFs=fs;
        setHDMITX_NCTS(hdmiTxDev[gHDMI_TxIndex].bAudFs); // set N, CTS by new generated clock.
        //CurrCTS=0;
        return;
    }
    return;
...
void setHDMITX_AudioChannelEnable(BOOL EnableAudio_b)
{
    ...
    if(EnableAudio_b)
    {
        if(AudioDelayCnt==0)
        {
            //if(hdmiTxDev[gHDMI_TxIndex].bAuthenticated==FALSE)
