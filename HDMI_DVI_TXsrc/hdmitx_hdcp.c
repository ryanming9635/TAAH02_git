...
//            printf("ER_FAIL == hdmitx_hdcp_Authenticate\n");
    ...
    {
        hdmiTxDev[gHDMI_Index].bAuthenticated=FALSE;
        hdmitx_hdcp_ResetAuth();
    }
    return TRUE ;
}
...
{
//    HDCP_DEBUG_PRINTF((" getHDMITX_AuthenticationDone() = %s\n",hdmiTxDev[gHDMI_Index].bAuthenticated?"TRUE":"FALSE" ));
    return hdmiTxDev[gHDMI_Index].bAuthenticated;
}
...
// Side-Effect:
//  1. hdmiTxDev[gHDMI_Index].bAuthenticated global variable will be TRUE when authenticated.
...
//            HDCP_DEBUG_PRINTF(("reg46 = %02x reg16 = %02x\n",(int)ucdata,(int)HDMITX_ReadI2C_Byte(0x16))); 
            ...
            {
                hdmiTxDev[gHDMI_Index].bAuthenticated = TRUE ;
				HDCP_DEBUG_PRINTF(("B_TX_AUTH_DONE\n"));
                break ;
            }
...
SYS_STATUS hdmitx_hdcp_VerifyIntegration()
{
    ...
    if(hdmiTxDev[gHDMI_Index].bAuthenticated == TRUE)
    {
        return ER_SUCCESS ;
    }
    return ER_FAIL ;
...
SYS_STATUS hdmitx_hdcp_Authenticate_Repeater()
{
    ...
    for(ii=0;ii<55;ii++)    //delayTX1ms(550); // emily add for test
    {
        if((B_TX_INT_HPD_PLUG|B_TX_INT_RX_SENSE)&HDMITX_ReadI2C_Byte(REG_TX_INT_STAT1))
        {
            goto hdmitx_hdcp_Repeater_Fail ;
...
{
//	printf("hdmitx_hdcp_ResumeAuthentication \n") ; 
    //setHDMITX_AVMute(TRUE);   //chunyu 
